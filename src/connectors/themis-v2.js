...
async function build(replicaSettings, clientSettings, log) {
  ...
  await promisified_spawn(cmd.proc, cmd.args, process.env.THEMIS_V2_DIR, log)
...
async function generateKeys(numKeys, log) {
  ...
  try {
    await fs.mkdir(
      path.join(process.env.THEMIS_V2_DIR, process.env.THEMIS_V2_KEYS_DIR),
    )
  } catch (error) {
  ...
  let cmd = {
    ...
    args: [
      ...
      numKeys,
      '--out-dir',
      process.env.THEMIS_V2_KEYS_DIR,
    ],
  }
  await promisified_spawn(cmd.proc, cmd.args, process.env.THEMIS_V2_DIR, log)
...
async function createConfigFile(replicaSettings, log) {
  ...
  let hostIPs = await ipUtil.getIPs({
    [process.env.THEMIS_V2_REPLICA_HOST_PREFIX]: replicaSettings.replicas,
    [process.env.THEMIS_V2_CLIENT_HOST_PREFIX]: 1, // FOR NOW?
  })
  for (let i = 0; i < hostIPs.length; i++) {
    if (hostIPs[i].name.startsWith(process.env.THEMIS_V2_REPLICA_HOST_PREFIX))
  ...
  for (let i = 0; i < hostIPs.length; i++) {
    ...
    config.peers.push({
      id: replicaId,
      host: hostIPs[i].ip,
      client_port: Number(process.env.THEMIS_V2_CLIENT_PORT),
      peer_port: Number(process.env.THEMIS_V2_REPLICA_PORT),
      private_key: `${process.env.THEMIS_V2_KEYS_DIR}/${auth.peers.pvKeyPrefix}${replicaId}`,
      public_key: `${process.env.THEMIS_V2_KEYS_DIR}/${auth.peers.pubKeyPrefix}${replicaId}`,
      certificate: `${process.env.THEMIS_V2_KEYS_DIR}/${auth.peers.certPrefix}${replicaId}`,
      prometheus_port: Number(process.env.THEMIS_V2_PROMETHEUS_PORT),
    })
    replicaId++
  }
  let pbft = TOML.parse(
  ...
  /* Execution Dir??? */
  await fs.writeFile(
    path.join(process.env.THEMIS_V2_DIR, process.env.THEMIS_V2_CONFIG_FILE_PATH),
    configString,
  )
  await fs.writeFile(
    path.join(process.env.THEMIS_V2_DIR, process.env.THEMIS_V2_PBFT_CONFIG_FILE_PATH),
    pbftString,
  )
...
async function passArgs(hosts, clientSettings, log) {
  ...
  for (let i = 0; i < hosts.length; i++) {
    if (hosts[i].isClient) {
      hosts[i].procs = []
      hosts[i].procs.push({
        path: path.join(process.env.THEMIS_V2_DIR, process.env.THEMIS_V2_CLIENT_BIN),
        env: 'RUST_LOG=INFO',
        args: `-d ${clientSettings.duration} --config ${process.env.THEMIS_V2_CONFIG_PATH} --payload ${clientSettings.payload} -c ${clientSettings.clients} --concurrent ${clientSettings.concurrent}`,
    ...
    hosts[i].procs = []
    hosts[i].procs.push({
      path: path.join(process.env.THEMIS_V2_DIR, process.env.THEMIS_V2_REPLICA_BIN),
      env: 'RUST_LOG=INFO',
      args: `${replicaIndex} --config ${process.env.THEMIS_V2_CONFIG_PATH}`,
      start_time: 0,
    })
    replicaIndex++
  }
...
function getExecutionDir() {
  return process.env.THEMIS_V2_EXECUTION_DIR
}
function getExperimentsOutputDirectory() {
  return process.env.THEMIS_V2_EXPERIMENTS_OUTPUT_DIR
}
...
async function getStats(experimentId, log) {
  let clientFile = await fs.readFile(
    path.join(
      path.join(process.env.THEMIS_V2_EXPERIMENTS_OUTPUT_DIR, experimentId),
      path.join(
        `hosts/${process.env.THEMIS_V2_CLIENT_HOST_PREFIX}0/${process.env.THEMIS_V2_CLIENT_HOST_PREFIX}0.bench-client.1000.stdout`,
      ),
    ),
  )
